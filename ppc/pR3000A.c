...
#define LW_SW_LR_COMN(swLR) \
    ...
    u32 *bAddrNull, *bChkDt, *bIdx1, *bIdx2, *bIdx3, *bEnd1, *bEnd2, *bEnd3, *bEndWriteDt, *bWriteAddrNull, *bWriteChkDt; \
     \
    STWU(r1, -0x20, r1); \
    STW(r28, 4, r1); \
    STW(r29, 8, r1); \
...
#define SW_LR_WRITEMEM(regIdx) \
    LIW(r28, (u32)(&psxMemWLUT)); \
    LWZX(r28, r28, r26); \
    CMPWI(r28, 0); \
    BEQ_L(bWriteAddrNull); \
    STWBRX(regIdx, r28, r27); \
    if (regIdx == 0) { \
        FlushAllHWReg(); \
    } \
    B_L(bEndWriteDt); \
    B_DST(bWriteAddrNull); \
    debugSWLR(regIdx); \
    B_DST(bEndWriteDt);
...
static void recSWL() {
    ...
    B_DST(bIdx3);
    SW_LR_WRITEMEM(GetHWReg32(_Rt_)); // (reg      ) | (mem & 0x00000000)
    //debugSWLR(GetHWReg32(_Rt_));
...
static void recSWR2() {
    ...
    SW_LR_WRITEMEM(GetHWReg32(_Rt_));
     // (reg      ) | (mem & 0x00000000)
    //debugSWLR(GetHWReg32(_Rt_));
    B_L(bEnd1);

    B_DST(bIdx1);
    SLWI(0, GetHWReg32(_Rt_), 8);
    ANDI_(r3Put, r3Get, 0xff);
    OR(0, 0, r3Get);
    ...
    B_DST(bIdx2);
    SLWI(0, GetHWReg32(_Rt_), 16);
    ANDI_(r3Put, r3Get, 0xffff);
    OR(0, 0, r3Get);
