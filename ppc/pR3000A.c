...
static void printLWLInfo(u32 addr, u32 val, u32 oldVal)
{
    #ifdef SHOW_DEBUG
    sprintf(txtbuffer, "recLWL %08x %d %08x %08x", addr, addr & 3, val, oldVal);
...
int DoShift(u32 k)
{
    ...
    for (i=0; i<30; i++) {
        ...
        {
            #ifdef SHOW_DEBUG
            sprintf(txtbuffer, "DoShift %ld %d", (s32)k, i);
...
#define LW_SW_LR_COMN(useR30) \
    if (!_Rt_) return; \
     \
    int rs; \
    u32 *bAddrNull, *bChkDt, *bIdx1, *bIdx2, *bIdx3, *bEnd1, *bEnd2, *bEnd3; \
     \
    STWU(r1, -0x10, r1); \
    STW(r28, 4, r1); \
    STW(r29, 8, r1); \
    ...
    SRWI(0, GetHWRegSpecial(ARG1), 16); \
    SLWI(0, 0, 2); \
    LWZX(r28, r28, 0); \
    CMPWI(r28, 0); \
    BEQ_L(bAddrNull); \
    ANDI_(PutHWRegSpecial(ARG1), GetHWRegSpecial(ARG1), 0xfffc); \
    if (useR30) { \
        MR(r30, GetHWRegSpecial(ARG1)); \
    } \
...
static void recLWR() {
    ...
    OR(PutHWReg32(_Rt_), GetHWRegSpecial(ARG1), 0); // (mem >>  8) | (reg & 0xff000000)
    FlushAllHWReg();
    B_L(bEnd2);
    ...
    OR(PutHWReg32(_Rt_), GetHWRegSpecial(ARG1), 0); // (mem >> 16) | (reg & 0xffff0000)
    FlushAllHWReg();
    B_L(bEnd3);
    ...
    OR(PutHWReg32(_Rt_), GetHWRegSpecial(ARG1), 0); // (mem >> 24) | (reg & 0xffffff00)
    FlushAllHWReg();
...
static void recSWL() {
    ...
    LIW(0, 0xffffff00);
    AND(0, GetHWRegSpecial(ARG1), 0);
    OR(0, r29, 0);
    STWBRX(0, r28, r30); // (reg >> 24) | (mem & 0xffffff00)
    FlushAllHWReg();
    B_L(bEnd1);

    B_DST(bIdx1);
    SRWI(0, GetHWReg32(_Rt_), 16);
    ANDIS(PutHWRegSpecial(ARG1), GetHWRegSpecial(ARG1), 0xffff);
    OR(0, 0, GetHWRegSpecial(ARG1));
    STWBRX(0, r28, r30); // (reg >> 16) | (mem & 0xffff0000)
    FlushAllHWReg();
    B_L(bEnd2);

    B_DST(bIdx2);
    SRWI(0, GetHWReg32(_Rt_), 8);
    ANDIS(PutHWRegSpecial(ARG1), GetHWRegSpecial(ARG1), 0xff00);
    OR(0, 0, GetHWRegSpecial(ARG1));
    STWBRX(0, r28, r30);  // (reg >>  8) | (mem & 0xff000000)
    FlushAllHWReg();
    B_L(bEnd3);

    B_DST(bIdx3);
    STWBRX(GetHWReg32(_Rt_), r28, r30); // (reg      ) | (mem & 0x00000000)
    FlushAllHWReg();
...
//                      (((gteIR2 >> 7) & 0x1f)<<5) |
//                      (((gteIR3 >> 7) & 0x1f)<<10);
//            // = ((gteIR1 >> 7) & 0x1f) | ((gteIR2 >> 2) & 0x3e0) | ((gteIR3 << 3) & 0x7c00);
//            psxRegs.GPR.r[_Rt_] = gteORGB;
            ...
            SRWI(0, 0, 2);
            ORI(0, 0, 0x3e0);
            OR(PutHWReg32(_Rt_), 0, GetHWReg32(_Rt_));
            ...
            STW(GetHWReg32(_Rt_), OFFSET(&psxRegs, &(gteORGB)), GetHWRegSpecial(PSXREGS));
            break;
...
//            gteSXYP = value;
            iFlushRegs(0);
            LWZ(0, OFFSET(&psxRegs, &(gteSXY1)), GetHWRegSpecial(PSXREGS));
            ...
            STW(GetHWReg32(_Rt_), OFFSET(&psxRegs, &(gteSXY2)), GetHWRegSpecial(PSXREGS));
...
//            gteIR3 = ((value >> 10) & 0x1f) << 7 = ((value      ) & 0x7c00) >> 3;
            STW(GetHWReg32(_Rt_), OFFSET(&psxRegs, &psxRegs.CP2D.r[28]), GetHWRegSpecial(PSXREGS));
            iFlushRegs(0);
            ORI(0, GetHWReg32(_Rt_), 0x1f);
            SLWI(0, 0, 7);
            STW(0, OFFSET(&psxRegs, &(gteIR1)), GetHWRegSpecial(PSXREGS));

            ORI(0, GetHWReg32(_Rt_), 0x3e0);
            SLWI(0, 0, 2);
            STW(0, OFFSET(&psxRegs, &(gteIR2)), GetHWRegSpecial(PSXREGS));

            ORI(0, GetHWReg32(_Rt_), 0x7c00);
            SRWI(0, 0, 3);
...
static void recCTC2() {
    ...
    switch (reg) {
        ...
        case 31:
            ...
            CMPWI(0, 0);
            BEQ_L(bEnd);
            ORIS(PutHWReg32(_Rt_), GetHWReg32(_Rt_), 0x8000);
            B_DST(bEnd);
            STW(GetHWReg32(_Rt_), OFFSET(&psxRegs, &psxRegs.CP2C.r[reg]), GetHWRegSpecial(PSXREGS));
            break;

        default:
            //psxRegs.CP2C.r[reg] = psxRegs.GPR.r[_Rt_];
            STW(GetHWReg32(_Rt_), OFFSET(&psxRegs, &psxRegs.CP2C.r[reg]), GetHWRegSpecial(PSXREGS));
            break;
    }
}
