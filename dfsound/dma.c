...
// if (crc == 0) *--pusPSXMem=0xFF;
    iSize <<= 1;
    if (spu.spuAddr + iSize > 0x7ffff)
    {
        int tmpAddr = 0x80000 - spu.spuAddr;
        memcpy(pusPSXMem, spu.spuMem + spu.spuAddr, tmpAddr);
        spu.spuAddr = 0;
    }
    else
    {
        memcpy(pusPSXMem, spu.spuMemC + spu.spuAddr, iSize);
        spu.spuAddr += iSize;
        spu.spuAddr &= 0x7fffe;
    }
}
...
//  }
    ...
    {
        int tmpAddr = 0x80000 - spu.spuAddr;
        memcpy(spu.spuMemC + spu.spuAddr, pusPSXMem, tmpAddr);
        spu.spuAddr = 0;
    }
    else
    {
        memcpy(spu.spuMemC + spu.spuAddr, pusPSXMem, iSize);
        spu.spuAddr += iSize;
        spu.spuAddr &= 0x7fffe;
    }
}
