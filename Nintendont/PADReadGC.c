...
static vu32* HID_CHANGE = (vu32*)0xD3003444;
static vu32* HID_CFG_SIZE = (vu32*)0xD3003448;
static vu32* HID_CFG_FILE = (vu32*)0xD3003460;
...
void HIDUpdateControllerIni()
{
	...
	{
		//snprintf(debugInfo, sizeof(debugInfo), "HID_CHANGE = 0 \r\n");
		//writeLogFile(debugInfo);
		return;
	}
	...
	if (f != NULL)
	{
		...
		*(vu32*)HID_CFG_SIZE = fsize;
		...
		//snprintf(debugInfo, sizeof(debugInfo), "load hid ini ok %04X \r\n", DeviceVID);
		//writeLogFile(debugInfo);
	}
	else
	{
		//snprintf(debugInfo, sizeof(debugInfo), "No controller \r\n");
		//writeLogFile(debugInfo);
	...
	else
	{
		for (chan = 0; (chan < MaxPads); ++chan)
		{
			...
			if(Pad[chan].button & 0x8000) /* controller not enabled */
			{
				...
				u32 psize = sizeof(PADStatusKernel)-1; //dont set error twice
