...
static inline u32 memRead32(u32 addr)
{
	u32 x;
...
static inline void memWrite32(u32 addr, u32 x)
{
...
{
	while((memRead32(0x80003140)) != rev)
		udelay(1000);

	u32 counter;
	for (counter = 0; !(memRead32(0x0d000004) & 2); counter++)
	{
		udelay(1000);
		if (counter >= 40000)
			break;
